...
class DeepSeekParser:
    ...
    def _lazy_load_model(self):
        ...
        if self._model is None:
            ...
            # Force CPU usage to bypass your GPU's 2GB memory wall
            logger.warning("Forcing CPU execution to prevent GPU OutOfMemory on 2GB VRAM") 
            self._model = self._model.eval().to("cpu").to(torch.bfloat16)
    ...
    def _extract_text_from_image_file(self, image_path: Path) -> str:
        ...
        try:
            # Fix Windows Path Errors by explicitly passing absolute strings 
            absolute_image_path = str(image_path.resolve())
            absolute_output_dir = str(image_path.parent.resolve())

            # Official API usage from transformers docs
            result = self._model.infer(
                self._tokenizer,
                prompt=prompt,
                image_file=absolute_image_path,  # Now explicitly absolute
                output_path=absolute_output_dir, # <-- CRITICAL: Prevents Windows from defaulting to ''
                base_size=config["base_size"],
                image_size=config["image_size"],
                crop_mode=True,
                save_results=False,  # Disables secondary image outputs safely
            )
